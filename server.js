/**
 * Tech Tool OPD Card + Follow-up + Coaching + Follow-up Note
 * Node 20+
 */
...
// Clinic templates
const CLINICS = {
  neuromed: { name:'Neurology Medicine',
    followup:{ default_window_days:28, tests:['CBC/CMP (ถ้าปรับยาใหม่)','FBS/HbA1c (ถ้ามี DM)','Lipid profile (ตามจำเป็น)'],
...
// Safe LLM call (ไม่ส่ง temperature ให้ o3/o1)
async function callLLM(model, input) {
  const payload = { model, input };
  if (!/^o\d/i.test(model)) payload.temperature = 0.2;
...
function buildFollowupTemplate(clinicKey,ctx='',risk='routine'){
  ...
  return { clinic:CLINICS[clinicKey]?.name||'Neurology', risk, days:Math.max(3,days), context:ctx,
    tests:f.tests, imaging:f.imaging||[], meds:f.meds, counsel:f.counsel, monitor:f.monitor, red:f.red, team:f.team, tele:f.tele };
}
function renderFollowupMarkdown(fp){
  const A=x=>Array.isArray(x)?x:[]; const out=[];
  out.push(`**แผนติดตาม (${fp.clinic})**`); out.push(`- ระดับความเสี่ยง: **${fp.risk.toUpperCase()}**`); out.push(`- นัดติดตามใน: **${fp.days} วัน**`);
  ...
  out.push(`\n**Telemedicine:** ${fp.tele?'เหมาะสม (OK)':'ไม่เหมาะสม'}`); return out.join('\n');
}
function buildCoachPrompt(rawText,clinicKey='neuromed'){
  const clinic=CLINICS[clinicKey]?.name||'Neurology'; const hint=CLINICS[clinicKey]?.promptHint||'';
  return `คุณเป็นแพทย์คลินิก ${clinic}.
...
ตอบเป็น JSON ตามคีย์:
{"patient_advice_md":"","clinician_brief_md":"","deep_plan_md":"",
...
function buildFollowupNotePrompt(rawText,clinicKey='neuromed'){
  const clinic=CLINICS[clinicKey]?.name||'Neurology';
  return `คุณเป็นแพทย์คลินิก ${clinic}.
สรุป "บันทึกการติดตาม (Follow-up Note)" จากข้อความต่อไปนี้ เป็น JSON:
{"chief_update":"","changes_since_last":[],"meds_update":[],"exam_focus":[],"assessment":"","plan":[],"return":{"when_days":14,"mode":"OPD"},"red_flags":[]}
ข้อความดิบ:
${String(rawText||'').trim()}`.trim();
}
...
app.post('/api/opd/from-text',async(req,res)=>{
  ...
  const models=forceModel==='o3'?['o3','gpt-4.1','gpt-4o-mini']:[OPENAI_MODEL,'gpt-4.1','gpt-4o-mini'];
  let lastErr; for(const m of models){ try{
    const r=await callLLM(m,prompt); const out=r.output_text?.trim?.()||r?.content?.[0]?.text?.trim?.()||'';
    if(out) return res.json({ok:true,model_used:m,clinicKey,summary:out});
  }catch(e){ lastErr=e; console.error('[from-text]',m,e?.status,e?.message);} }
...
app.post('/api/opd/upload-audio',upload.single('audio'),async(req,res)=>{
  try{
    ...
    let text=''; try{ const tr=await openai.audio.transcriptions.create({file,model:TRANSCRIBE_MODEL,language:'th'}); text=tr?.text?.trim?.()||''; }
    catch{ const tr=await openai.audio.transcriptions.create({file,model:'whisper-1',language:'th'}); text=tr?.text?.trim?.()||''; }
    if(!text) return res.status(500).json({ok:false,error:'transcription empty'});
    const r=await callLLM(OPENAI_MODEL, buildThaiPrompt(text,clinicKey));
    const summary=r.output_text?.trim?.()||r?.content?.[0]?.text?.trim?.()||'';
    res.json({ok:true,clinicKey,transcript:text,summary});
  }catch(e){ console.error('[upload-audio]',e?.status,e?.message); res.status(500).json({ok:false,error:'upload failed',message:e?.message}); }
});
...
app.post('/api/followup/from-text',(req,res)=>{
  ...
  res.json({ok:true,structured:plan,markdown:renderFollowupMarkdown(plan)});
});
...
app.post('/api/coach',async(req,res)=>{
  ...
  const models=forceModel==='o3'?['o3','gpt-4.1','gpt-4o-mini']:[OPENAI_MODEL,'gpt-4.1','gpt-4o-mini'];
  let lastErr; for(const m of models){ try{
    const r=await callLLM(m,prompt); const txt=r.output_text?.trim?.()||r?.content?.[0]?.text?.trim?.()||''; const clean=txt.replace(/```json|```/g,'').trim(); const json=JSON.parse(clean);
    return res.json({ok:true,model_used:m,...json});
  }catch(e){ lastErr=e; console.error('[coach]',m,e?.status,e?.message);} }
  res.status(500).json({ok:false,error:'coach failed',message:lastErr?.message});
});

// Follow-up Note (timeline item)
app.post('/api/followup/note',async(req,res)=>{
  const {rawText='',clinicKey='neuromed',forceModel}=req.body||{};
  if(!rawText.trim()) return res.status(400).json({ok:false,error:'no text'});
  const prompt=buildFollowupNotePrompt(rawText,clinicKey);
  const models=forceModel==='o3'?['o3','gpt-4.1','gpt-4o-mini']:[OPENAI_MODEL,'gpt-4.1','gpt-4o-mini'];
  let lastErr; for(const m of models){ try{
    const r=await callLLM(m,prompt); const txt=r.output_text?.trim?.()||r?.content?.[0]?.text?.trim?.()||''; const clean=txt.replace(/```json|```/g,'').trim(); const json=JSON.parse(clean);
    return res.json({ok:true,model_used:m,note:json});
  }catch(e){ lastErr=e; console.error('[followup/note]',m,e?.status,e?.message);} }
  res.status(500).json({ok:false,error:'followup-note failed',message:lastErr?.message});
});
